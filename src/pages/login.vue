<template>
  <div class="login-warp" :style="{height:type==='login'?'787px':'1000px'}">
    <div class="content">
      ...
      <div class="box">
        <div class="form">
          <div class="title" v-if="type==='login'">欢迎登录</div>
          <div class="title" v-if="type==='reg'">欢迎注册</div>
          ...
          <div class="item" v-if="type==='reg'">
            <div class="label">邀请码</div>
            <el-input v-model="inviteCode" placeholder="请输入邀请码" prefix-icon="el-icon-unlock"></el-input>
          </div>
          <div class="for_reg" v-if="type==='login'">
            <router-link class="forget" tag="div" to="/forget">忘记密码？</router-link>
            <router-link class="reg" tag="a" to="/register">注册账号</router-link>
          </div>
          <div class="for_reg" v-else>
            <span class="forget"></span>
            <router-link class="reg" tag="a" to="/login">现有账号登录</router-link>
          </div>
          <div class="btn" v-if="type==='login'" @click="login">登录</div>
          <div class="btn" v-if="type==='reg'" @click="register">注册</div>
          <div class="btn" v-if="type==='forget'" @click="refound">找回密码</div>
        </div>
        <div class="tip">
